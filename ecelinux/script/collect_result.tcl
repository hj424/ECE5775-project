...
# collect-result.tcl 
#============================================================================
# @brief: A Tcl script that collect and dump out results from rendering csim & synthesis.
#         The results will be saved under folder ./result/
# @desc:
# 1. print out header infomation if the result file is newly created
# 2. out.dat is reserved for further storage
...
if { ! [ file exists "./result/${filename}"] } {
  ...
  if { [ file exist "${hls_prj}/solution1/sim/report/rendering_cosim.rpt" ] } {
   set msg "Design Name CP BRAM DSP FF LUT Latency cosim-Latency"
  } else {
    set msg "Design Name CP BRAM DSP FF LUT Latency"
  }
  puts $fileId $msg
  close $fileId
}
...
if { [ file exist "${hls_prj}/solution1/sim/report/rendering_cosim.rpt" ] } {
  set fp [open "${hls_prj}/solution1/sim/report/rendering_cosim.rpt" r]
  set file_data [read $fp]
  close $fp
  set data [split $file_data "\n"]
  foreach { pattern } { 
